...
void IBSink::consumeDataMsg(IBDataMsg *p_msg)
{
  ...
  {
    EV << "-I- "
       << "finished packet" << p_msg->getMsgIdx() << ":" << packet_counter_ << omnetpp::endl;
  }
...
void IBSink::handleMessage(omnetpp::cMessage *p_msg)
{
  ...
  case 1:
    ...
    break; // in the case of IB_DATA_MSG
  case 2:
    EV << "-I- " << getFullPath() << " Dropping flow control message";
    delete p_msg;
    break; // in the case of IB_FLOWCTRL_MSG
  case 7:
    handlePop(p_msg);
    break; // in the case of IB_POP_MSG
  case 8:
    handleHiccup(p_msg);
    break; // in the case of IB_HICCUP_MSG
  case 10:
    delete p_msg;
    break; // in the case of IB_DONE_MSG
  case 19:
    handleSinkTimer((IBSinkTimerMsg *)p_msg);
    break; // in the case of IB_SINKTIMER_MSG
  default:
    error("-E- %s does not know what to with msg: %d is local: %d"
          " senderModule: %s",
          getFullPath().c_str(), p_msg->getKind(),
