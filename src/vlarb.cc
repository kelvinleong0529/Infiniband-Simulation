                          ...
                          unsigned int &curPortNum, unsigned int &curVl)
{
  ...
  for (unsigned int i = 0; i <= maxVL + 1; i++)
  {
    ...
    if (!Tbl[idx].weight || (Tbl[idx].used > Tbl[idx].weight))
      continue;
...
void IBVLArb::handleMessage(omnetpp::cMessage *p_msg)
{
  ...
  case 4:
    ...
    break; // in the case of IB_TXCRED_MSG
  case 7:
    handlePop();
  ...
  case 9:
    delete p_msg;
    arbitrate();
    break; // in the case of IB_FREE_MSG
  case 10:
    delete p_msg;
    arbitrate();
    break; // in the case of IB_DONE_MSG
  default:
    error("-E- %s does not know how to handle message: %d", getFullPath().c_str(), p_msg->getKind());
    delete p_msg;
  }
}
